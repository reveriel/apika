...
std::set<const char*> nativeClassName;
std::set<const char*> allClassName;
std::set<const char*> libClassName;
std::set<u4> allMethod;
std::set<const char*> nativeClass;
std::set<u4> nativeMethod;
...
struct Options {
    ...
    bool showMethods;
    bool showLibMethods;
    bool showUserMethods;
    bool showNativeMethods;
};
    ...
    int insnWidth, const DecodedInstruction* pDecInsn)
{
    ...
    if (gOptions.showMethods){
    	if (pDecInsn->indexType == kIndexMethodRef){	    
	    /* TODO: Make the index *always* be in field B, to simplify this code. */
	        switch (dexGetFormatFromOpcode(pDecInsn->opcode)) {
	        case kFmt20bc:
	        case kFmt21c:
            ...
            FieldMethodInfo methInfo;
            if (getMethodInfo(pDexFile, index1, &methInfo)) {			
                std::set<u4>::iterator SJ = allMethod.find(index1);
                if(SJ == allMethod.end()){
                    allMethod.insert(index1);
                }
                std::set<const char*>::iterator SI = allClassName.find(methInfo.classDescriptor);		
                if(SI == allClassName.end()){
                    allClassName.insert(methInfo.classDescriptor);
	        } 
            } 
        }        
    }else{
...
void dumpBytecodes(DexFile* pDexFile, const DexMethod* pDexMethod)
{
    ...
    std::set<u4>::iterator SI = allMethod.find(pDexMethod->methodIdx);
    if(SI == allMethod.end())
        allMethod.insert(pDexMethod->methodIdx);

    if ((pDexMethod->accessFlags & ACC_NATIVE) != 0){
        std::set<u4>::iterator SJ = nativeMethod.find(pDexMethod->methodIdx);
        if(SJ == nativeMethod.end())
            nativeMethod.insert(pDexMethod->methodIdx);
    }
...
void dumpClass(DexFile* pDexFile, int idx, char** pLastPackage)
{
    ...
    if (gOptions.showMethods){
        std::set<const char*>::iterator SI = nativeClassName.find(classDescriptor);
        if(SI == nativeClassName.end())
    	    nativeClassName.insert(classDescriptor); 

        for (i = 0; i < (int) pClassData->header.directMethodsSize; i++) {
		dumpBytecodes(pDexFile, &pClassData->directMethods[i]);             
	    }

        for (i = 0; i < (int) pClassData->header.virtualMethodsSize; i++) {
		dumpBytecodes(pDexFile, &pClassData->virtualMethods[i]);
	    }
    }else{
	    if (gOptions.outputFormat == OUTPUT_PLAIN) {
		printf("Class #%d            -\n", idx);
...
void processDexFile(const char* fileName, DexFile* pDexFile)
{
    ...
    for (i = 0; i < (int) pDexFile->pHeader->classDefsSize; i++) {
     ...
     if (gOptions.showMethods) { 
        ...
        for (std::set<const char*>::iterator sR = allClassName.begin(), eR = allClassName.end(); sR != eR; ++sR) {                      
            std::set<const char*>::iterator SI = nativeClassName.find(*sR);
            std::set<const char*>::iterator SJ = libClassName.find(*sR);        
            if ((SI == nativeClassName.end()) && (SJ == libClassName.end())){		
                libClassName.insert(*sR);             
	    }
        }
        
        for (std::set<u4>::iterator sA = allMethod.begin(), eA = allMethod.end(); sA != eA; ++sA) { 
            u4 index = *sA;                                 
            if (getMethodInfo(pDexFile, index, &methInfo)) {
                const char* string1 = "loadLibrary";
                if (strcmp(methInfo.name,string1) == 0){
                    printf("The loadLibrary function:\n");
                    printf("%s.%s:%s\n",methInfo.classDescriptor, methInfo.name,methInfo.signature);
                    const DexMethodId* pMethodId = dexGetMethodId(pDexFile, index);
                    const DexFile* pDexFile1 = pDexFile;
                    dexProtoSetFromMethodId(&pProto, pDexFile1, pMethodId);
                    size_t libParameterCount = dexProtoGetParameterCount(&pProto);
                    dexStringCacheInit(&pCache);
                    const char* libParameterName = dexProtoGetParameterDescriptors(&pProto,&pCache);
                  //  printf("The LoadLibrary has %d parameters and the loaded library is %s.\n",libParameterCount, libParameterName);
                }
                free((void *) methInfo.signature);
             }
        }
    ...
    //输出本地方法
    if (gOptions.showNativeMethods) {    
        for (std::set<u4>::iterator sN = nativeMethod.begin(), eN = nativeMethod.end(); sN != eN; ++sN) { 
            u4 index = *sN;                                 
            if (getMethodInfo(pDexFile, index, &methInfo)) {
                std::set<const char*>::iterator SI = nativeClass.find(methInfo.classDescriptor);
                if (SI == nativeClass.end()) 
                    nativeClass.insert(methInfo.classDescriptor);
                printf("%s.%s:%s\n",methInfo.classDescriptor, methInfo.name,methInfo.signature);
                free((void *) methInfo.signature);
                nmt++;
             }
        }
    printf("There are %d native methods!\n",nmt);

    for(std::set<const char*>::iterator sL = nativeClass.begin(), eL = nativeClass.end(); sL != eL; ++sL) {            
            printf("%s\n",*sL);
            nct++;
        }
        printf("There are %d native class!\n",nct);

    }

    //输出库方法
    if (gOptions.showLibMethods) {    
        for (std::set<u4>::iterator sA = allMethod.begin(), eA = allMethod.end(); sA != eA; ++sA) { 
            u4 index = *sA;                                 
            if (getMethodInfo(pDexFile, index, &methInfo)) {
                std::set<const char*>::iterator SI = nativeClassName.find(methInfo.classDescriptor);
                if (SI == nativeClassName.end()){
                    printf("%s.%s:%s\n",methInfo.classDescriptor, methInfo.name,methInfo.signature);
                    lmt++;
                }
        ...
        printf("There are %d lib methods!\n",lmt);

        for(std::set<const char*>::iterator sL = libClassName.begin(), eL = libClassName.end(); sL != eL; ++sL) {            
            printf("%s\n",*sL);
            lct++;
        }
        printf("There are %d lib class!\n",lct);
    }
    
    //输出程序自定义方法
    if (gOptions.showUserMethods) {    
        for (std::set<u4>::iterator sA = allMethod.begin(), eA = allMethod.end(); sA != eA; ++sA) { 
            u4 index = *sA;                                 
            if (getMethodInfo(pDexFile, index, &methInfo)) {
                std::set<const char*>::iterator SI = libClassName.find(methInfo.classDescriptor);
                if (SI == libClassName.end()){
                    printf("%s.%s:%s\n",methInfo.classDescriptor, methInfo.name,methInfo.signature);
                    umt++;
                }
                free((void *) methInfo.signature);
        ...
        printf("There are %d user define methods!\n",umt);

        for(std::set<const char*>::iterator sL = nativeClassName.begin(), eL = nativeClassName.end(); sL != eL; ++sL) {            
            printf("%s\n",*sL);
            uct++;
        }
        printf("There are %d user define class!\n",uct);
    }
...
void usage(void)
{
    ...
    fprintf(stderr,
        "%s: [-c] [-d] [-a] [-k] [-s] [-n] [-i] [-f] [-h] [-i] [-l layout] [-m] [-t tempfile] dexfile...\n",
        gProgName);
    fprintf(stderr, "\n");
    fprintf(stderr, " -c : verify checksum and exit\n");
    fprintf(stderr, " -d : disassemble code sections\n");
    fprintf(stderr, " -a : dump methods\n");
    fprintf(stderr, " -k : dump lib methods\n");
    fprintf(stderr, " -s : dump user define methods\n");
    fprintf(stderr, " -n : dump native methods\n");
...
int main(int argc, char* const argv[])
{
    ...
    while (1) {
        ic = getopt(argc, argv, "cdaksnfhil:mt:");
        if (ic < 0)
            break;
        ...
        case 'd':       // disassemble Dalvik instructions
            ...
            break;
        case 'a':       // dump methods
            gOptions.showMethods = true;
            break;
        case 'k':       // dump lib methods
            gOptions.showLibMethods = true;
            break;
        case 's':       // dump user define methods
            gOptions.showUserMethods = true;
            break;
        case 'n':       // dump native methods
            gOptions.showNativeMethods = true;
            break;
